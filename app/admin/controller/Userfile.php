...
class Userfile extends AdminBase {
    ...
    protected function initialize(){
        parent::initialize();
        $models = "app\\common\\model\\UserFile";
    ...
    protected function getConf(){
        ...
        if($this->request->action() == 'index'){
            if ($this->contauth['create']) {
                $this->list_base['add'] = ['title' => '添加字段','class' => 'cx-button-s cx-bg-green'];
            }
            if($this->contauth['edit']){
                $this->list_file[] = ['filed' => 'edit','title' => '编辑','type' => 'btn','text' => true,'open' => true,'opentitle' => "编辑参数分类",'uri' => url('edit',['id' => '__id__'])->build(),'icon' => 'cx-iconbianji3 cx-text-f16','class' => 'cx-text-green','textalign' => 'center','width' => '80','fixed' => 'right'];
            }
            ...
            $this->form_list = [
                ...
                ['file' => 'sql_file','title' => '字段键值','type' => 'text','tip' => '只能是字母或字母加数字,并且禁止修改','required' => true],
                ['file' => 'sql_type','title' => '字段类型','type' => 'select','data' => ['list' => ["varchar(255) DEFAULT NULL" => "255个字符串以内","int(11) NOT NULL DEFAULT '0'" => "10位以内纯数字","tinyint(2) NOT NULL DEFAULT '0'" => "2位以内纯数字","decimal(10,2) unsigned NOT NULL DEFAULT '0'" => "价格","text NOT NULL" => "常用文本文档","mediumtext DEFAULT NULL" => "大型文本文档",],'default' => ''],'tip' => '选择后禁止修改，容易出错','required' => true],
                ...
                ['file' => 'list_show','title' => '列表页显示','type' => 'radio','data' => ['list' => ['1' => '显示','0' => '隐藏'],'default' => '0']],
                ['file' => 'cont_show','title' => '内容页显示','type' => 'radio','data' => ['list' => ['1' => '显示','0' => '隐藏'],'default' => '1']],
            ...
            $this->list_nav = [];
            $this->list_top = [
                ['uri' => url('index')->build(),'title' => '用户列表','class' => 'cx-button cx-bg-blue'],
                ['uri' => url('trashone')->build(),'title' => '批量还原','ids' => true,'class' => 'cx-button cx-bg-blue cx-mag-lr10'],
                ['uri' => url('pdel')->build(),'title' => '批量删除','ids' => true,'class' => 'cx-button cx-bg-red'],
            ];
        }
    }
    //  保存数据
    public function save(){
        ...
        try {
            validate($this->validate)->scene($this->request->isPut() ? "userfileedit" : "userfileadd")->check($data);
